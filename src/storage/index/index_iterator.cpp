...
INDEXITERATOR_TYPE::IndexIterator(page_id_t page_id, BufferPoolManager *buffer_pool_manager, int index)
    : _pageId(page_id), _bufferPoolManager(buffer_pool_manager), _index(index) {
...
INDEXITERATOR_TYPE::~IndexIterator() {
  if (_bufferPoolManager != nullptr && _pageId != INVALID_PAGE_ID) {
    _bufferPoolManager->UnpinPage(_pageId, false);
  }
}

INDEX_TEMPLATE_ARGUMENTS
bool INDEXITERATOR_TYPE::isEnd() { return _pageId == INVALID_PAGE_ID; }

INDEX_TEMPLATE_ARGUMENTS
const MappingType &INDEXITERATOR_TYPE::operator*() { return _curNode->GetItem(_index); }
